...
export default function FakeDBTrackingResults({ match }) {
  ...
  const defaltData = information.filter(
    (parcel) =>
      removeSpaceAndlower(parcel.user_name) === removeSpaceAndlower(userName)
  );
  const [cards, setCards] = useState(sortAndCreatCards("last_updated"));
